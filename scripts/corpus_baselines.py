...
def year_stats(papers: list[dict], hyper_author_threshold: int) -> dict:
    n = len(papers)
    ...
    hyper = [p for p in papers if p["n_authors"] > hyper_author_threshold]
    return {
        "papers": n,
        ...
        "hyper_authored_papers": len(hyper),
        "max_authors": max((p["n_authors"] for p in papers), default=0),
    }
...
def main() -> int:
    ...
    ap.add_argument("--hyper-author-threshold", type=int, default=20,
                    help="papers above this are reported separately (docs/09 §2)")
    args = ap.parse_args()
    ...
    stats = {y: year_stats(by_year[y], args.hyper_author_threshold)
             for y in sorted(by_year)}

    header = (f"{'year':<6}{'papers':>7}{'refs/pp':>9}{'auth/pp':>9}"
              f"{'authors':>9}{'degree':>8}{'frac deg':>10}{'hyper':>7}")
    print(header)
    print("-" * len(header))
